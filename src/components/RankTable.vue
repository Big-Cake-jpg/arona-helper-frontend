<template>
  <div class="grid grid-cols-2 gap-4 pt-5 pb-5">
    <mdui-text-field
      v-if="token"
      label="ID"
      @input="searchUID = $event.target.value"
      placeholder="815797"
      clearable
      ><mdui-icon-numbers slot="icon"></mdui-icon-numbers
    ></mdui-text-field>
    <mdui-text-field
      v-if="token"
      label="角色名"
    ...
    <mdui-checkbox @change="reverse = $event.target.checked" :checked="reverse"
      >倒序排序</mdui-checkbox
    >
  </div>
  <mdui-button
    ...
    :loading="queryButtonDisabled"
    ><mdui-icon-search slot="icon"></mdui-icon-search>搜索</mdui-button
  >
  <h2>排行榜</h2>
  <div class="text-center mt-5 h-xl" v-if="isRankFetchedFailed">
    ...
    <mdui-button
      ...
      :disabled="queryButtonDisabled"
      :loading="queryButtonDisabled"
      ><mdui-icon-search slot="icon"></mdui-icon-search>重新搜索</mdui-button
    >
  </div>
        ...
        <tbody>
          <tr v-for="item in data" :key="item.uid">
            ...
            <td class="align-middle">
              <mdui-avatar
                :src="`${apiRoot}/account/avatar?uid=${item.uid}`"
                fit="scale-down"
                label="User Avatar"
              ></mdui-avatar>
            </td>
